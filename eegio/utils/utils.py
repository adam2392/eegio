...
from base.utils.data_structures_utils import NumpyEncoder
...
def pair_name_to_infiles(config):
    # pats = [f for f in os.listdir(config['rawdatadir']) if
    #                 os.path.isdir(os.path.join(config['rawdatadir'], f))]
    # get all *.fif files recursively under directory A
    fif_path = Path(config['rawdatadir']).glob(
        "*/{}/fif/*.fif".format(config['modality']))

    # pair fif name to infile path using a dictionary
    # fif_infiles_dict_fif = {}
    fif_infiles_dict_json = {}
    # fif_infiles_dict_patient = {}
