...
class Electrode(object):
    def __init__(self, contact_list: List = None,
                 elec_type: str = None,
    ...
    def _separate_electrode_name_number(self, contact_list):
        ...
        for contact in contact_list:
            ...
            if len(match.groups()) != 2:
                raise AttributeError(f"Contact {contact} in contact_list attribute "
        ...
        if len(electrode_names) > 1:
            raise AttributeError(f"For this electrode object, you passed in multiple electrodes "
    ...
    def _check_data(self):
        ...
        contacts, electrode_names, contact_numbers = self._separate_electrode_name_number(self.contact_list)
    ...
    def _process_out_contacts(self):
        ...
        contacts, electrode_names, contact_numbers = self._separate_electrode_name_number(self.out_contacts)
        ...
        expected_contact_numbers = range(min(contact_numbers), MAX_CONTACTS_IN_ELECTRODE + 1)
    ...
    def _process_wm_contacts(self):
        # make sure if contacts ar outside, then everything after is outside
        contacts, electrode_names, contact_numbers = self._separate_electrode_name_number(self.wm_contacts)

    def _process_bad_contacts(self):
        contacts, electrode_names, contact_numbers = self._separate_electrode_name_number(self.bad_contacts)

    def _get_good_contacts(self):
        good_contacts = []

        # create a unionized list of all the contacts to exclude
        self.bad_contacts = list(set(self.bad_contacts).union(self.wm_contacts).union(self.out_contacts))

        # go through and set everything
        self.good_contacts = set(self.contact_list).difference(self.bad_contacts)
