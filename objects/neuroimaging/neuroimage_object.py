...
from eztrack.eegio.objects import pial_to_verts_and_triangs, read_cortical_region_mapping, \
    Hemisphere, RegionIndexMapping

SUBCORTICAL_REG_INDS = [8, 10, 11, 12, 13, 16,
                        17, 18, 26, 47, 49, 50, 51, 52, 53, 54, 58]
...
class StructuralDataset:
    ...
    def save_to_txt_zip(self, filename: os.PathLike):
        ...
        file_orientations = os.path.join(
            tmpdir.name, 'average_orientations.txt')
        ...
        with ZipFile(filename, 'w') as zip_file:
            ...
            zip_file.write(file_orientations,
                           os.path.basename(file_orientations))
    ...
    def compute_triangle_normals(triangles, vertices):
        ...
        try:
            triangle_normals = tri_norm / \
                np.sqrt(np.sum(tri_norm ** 2, axis=1))[:, np.newaxis]
    ...
    def compute_vertex_normals(number_of_vertices, vertices, vertex_triangles,
        ...
        for k in range(number_of_vertices):
            try:
                ...
                vert_norms[k, :] = vert_norms[k, :] / \
                    np.sqrt(np.sum(vert_norms[k, :] ** 2, axis=0))
                ...
                vert_norms[k, :] = vertices[k] / \
                    np.sqrt(vertices[k].dot(vertices[k]))
                bad_normal_count += 1
        if bad_normal_count:
...
class GetSurface():
    ...
    def get_subcortical_surfaces(subcort_surf_direc: os.PathLike, region_index_mapping: RegionIndexMapping) -> Surface:
        ...
        for fs_idx in SUBCORTICAL_REG_INDS:
            ...
            filename = os.path.join(
                subcort_surf_direc, 'aseg_%03d.srf' % fs_idx)
            with open(filename, 'r') as f:
                f.readline()
                nverts, ntriangs = [int(n)
                                    for n in f.readline().strip().split(' ')]

            vertices = np.genfromtxt(
                filename, dtype=float, skip_header=2, skip_footer=ntriangs, usecols=(0, 1, 2))
    ...
    def merge_surfaces(surfaces: List[Surface]) -> Surface:
        ...
        :return: Surface
        """
        offsets = np.cumsum([0] + [vs.shape[0]
                                   for vs in [surf.vertices for surf in surfaces]][:-1])
        vertices = np.vstack([surf.vertices for surf in surfaces])
        triangles = np.vstack(
            [ts + offset for ts, offset in zip([surf.triangles for surf in surfaces], offsets)])
    ...
    def compute_region_params(surface: Surface, subcortical: bool = False) \
        ...
        centers = GetSurface.compute_region_centers(
            regions, verts, region_mapping)
    ...
    def compute_region_orientations(regions, vertex_normals, region_mapping):
        ...
        for i, k in enumerate(regions):
            ...
            if orient.shape[0] > 0:
                ...
                average_orientation[i, :] = avg_orient / \
                    np.sqrt(np.sum(avg_orient ** 2))
    ...
    def compute_region_areas(regions, triangle_areas, vertex_triangles, region_mapping):
        ...
        for i, k in enumerate(regions):
            ...
            if region_triangles:
                region_surface_area[i] = triangle_areas[list(
                    region_triangles)].sum()
