...
class ConvertEDF(BaseConverter):
    ...
    Examples
    --------
    >>> from eegio.format.edfconversion.convertedf import ConvertEDF
    >>> datatype = 'ieeg'
    >>> filepath = './test.edf'
    ...
    >>> fif_rawdata = converter.convert_fif(bad_chans_list, fpath, save=True, replace=True)
    >>>
    >>> # convert metadata
    >>> clinical_json = {
    ...        'clinical_center': 'nih',
    ...
    def convert_fif(self, bad_chans_list=[], newfilepath=None, save=True, replace=False):
        ...
        :param replace: (optional; bool) to overwrite if existing fpath is already saved
