    ...
    def load_edf_files(self):
        for edffile in self.filelist:
            ...
            rawfif = edfconverter.convert_fif(
                bad_chans_list=[], save=False, replace=False
            )
            metadata = edfconverter.convert_metadata(
                patid, dataset_id, center, save=False
            )
    ...
    def combine_metadata(self):
        for idx, metadata in enumerate(self.metadata_list):
            if idx == 0:
                ...
                combined_metadata = self._update_dict(combined_metadata, metadata)
        return combined_metadata

    def _update_dict(self, master_dict, appendage_dict):
        TIME_DEPENDENT_KEYS = ["length_of_recording", "events", "onset", "termination"]
        ...
        for key in appendage_dict.keys():
            if key in TIME_DEPENDENT_KEYS:
                if key == "length_of_recording":
                    ...
                    master_dict[key] = self._concat_events(
                        master_dict[key], appendage_dict[key], prevsec
                    )
