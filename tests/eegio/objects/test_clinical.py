...
import pathlib
import inspect
from eegio.base.objects.clinical import MasterClinicalSheet, DataSheet

# @pytest.mark.usefixture('clinical_sheet')
class TestClinical():
    clinical_excel_fpath =  os.path.join(os.getcwd(), "./data/test_clinicaldata.xlsx")
    clinical_csv_fpath =  os.path.join(os.getcwd(), "./data/test_clinicaldata.csv")
    ...
    def test_clinical_conversion(self):
        ...
        pd.testing.assert_frame_equal(temp_csv_df, csv_df,
                                      check_dtype=True,
    ...
    def test_clinical(self):
        ...
        # check attributes
        attributes = inspect.getmembers(clinloader, lambda a: not (inspect.isroutine(a)))
        clin_attribs = [a for a in attributes if not (a[0].startswith('__') and a[0].endswith('__'))]
        print(clin_attribs)
